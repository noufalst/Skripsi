...
bromelia = PlantParameters(
    ...
    r_stoma_min = 47.2,     # [C8] REAL — percentile 5% dari Licor CAM malam (Maret-April 2026)
)
...
def load_weather_data(filepath: str,
    ...
    # File adalah .xlsx (bukan CSV). Ada 3 baris header (row0=Column1..., row1=Temp/Hi/..., row2=Date/Time/...)
    # Data mulai baris ke-4 (skiprows=3). Format tanggal: DD/MM/YY (dikonfirmasi dari data)
    ...
    # Buat timestamp — format DD/MM/YY (dikonfirmasi: '24/04/26' = 24 April)
...
def compute_substrate_properties(theta: np.ndarray,
    ...
    # [FIX Bug8] lambda_g = lambda_dry + theta * lambda_water (W/mK)
    # NOT theta * k_theta (hydraulic conductivity, m/s — completely wrong unit)
    lambda_g = substrate.lambda_dry + theta * substrate.lambda_water      # B.9
...
def solve_substrate_heat(T_g: np.ndarray,
    ...
    # [FIX Bug7] r_vap: vapor resistance substrat — bukan lambda*50 (salah unit)
    # Formula: r_vap = 10 + 1990*(1 - theta_rel)²  (Lee & Sim 2012)
    theta_rel = float(np.clip(theta[0] / substrate.theta_sat, 0, 1))
    r_vap = 10 + 1990 * (1 - theta_rel)**2
    r_a   = compute_aerodynamic_resistance(u, plant)
...
def solve_substrate_moisture(theta: np.ndarray,
    ...
    r_a     = compute_aerodynamic_resistance(u, plant)
    # [FIX Bug6] pass actual G_sol (was hardcoded 0 → CAM always nighttime)
    ...
    # Clip to zero: no condensation in this model (only evaporation)
    ...
    # Convert to [m/s] for Richards equation (VWC is dimensionless m³/m³)
    # j_flux [m/s] = j_eva [kg/m²s] / rho_water [1000 kg/m³]
    rho_water   = 1000.0
    j_eva_ms    = j_eva / rho_water          # [m/s] — for moisture BC
    j_irrig_ms  = j_irrigation / rho_water   # [m/s] — irigasi juga dikonversi

    aw = np.zeros(Nz); ap = np.zeros(Nz)
    ae = np.zeros(Nz); bv = np.zeros(Nz)

    for i in range(1, Nz-1):
        Dw = 0.5*(abs(D_theta[i-1])+abs(D_theta[i]))
        De = 0.5*(abs(D_theta[i])+abs(D_theta[i+1]))
    ...
    # FIX: same sign convention as substrate heat — negate off-diagonal
...
def run_simulation(weather_df: pd.DataFrame,
    ...
    # Q_gain — Persamaan (18)
    Q_gain = float(np.trapezoid(results['q_s_in'], dx=60.0))
