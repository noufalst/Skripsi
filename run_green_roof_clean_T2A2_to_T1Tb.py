...
VALIDATION_TARGETS: Dict[str, str] = {
    # Kalau T1Tb = indoor roof CAM dan T2A2 = outdoor roof CAM,
    # target validasi T_s_in CAM harus T1Tb. T2A2 dipakai sebagai top boundary forced slab.
    "CAM": "T1Tb",
    "C3": "T3Ka",
}
...
def run_cam_workflow() -> Tuple[dict, dict, pd.DataFrame]:
    ...
    if RUN_FORCED_SLAB_DIAGNOSTIC:
        diagnostic_cases = [
            # Main check kalau mapping sensor yang benar adalah:
            # T2A2 = outdoor/top side CAM, T1Tb = indoor/bottom side CAM.
            ("T2A2", "T1Tb", "T1Ka", "forced_slab_T2A2_to_T1Tb_OUTDOOR_to_INDOOR.png"),

            # Pembanding lama, berguna kalau ternyata T1Ta juga kandidat outdoor roof CAM.
            ("T1Ta", "T1Tb", "T1Ka", "forced_slab_T1Ta_to_T1Tb.png"),

            # Diagnostic tambahan lama, jangan dijadikan target utama sebelum mapping jelas.
