"""
================================================================================
GREEN ROOF CAM-ONLY PHYSICAL MODEL — LI-COR r_s VERSION
================================================================================
Reference model:
...
Purpose:
    ...
    This version explicitly uses LI-COR stomatal conductance (gsw) as stomatal
    resistance r_s / r_stoma, so gas-exchange data affects latent cooling.

Core difference from reduced calibration versions:
    LI-COR gsw -> r_s -> h_eva_f -> j_eva_f -> energy/moisture balance
...
class CAMRsProfile:
    """
    CAM phase stomatal resistance profile derived from LI-COR gsw.

    r_s_night_s_m:
        Phase I, stomata open at night.
    r_s_midday_s_m:
        Phase III, daytime closure / high resistance.
    r_s_late_afternoon_s_m:
        Phase IV, partial reopening in late afternoon.
    """
    r_s_night_s_m: float = 90.0
    r_s_midday_s_m: float = 500.0
    r_s_late_afternoon_s_m: float = 197.0
    source_summary: str = "defaults; replace with LI-COR-derived values when available"
...
def _read_rika_csv(filepath: Union[str, Path]) -> pd.DataFrame:
    """
    Read RIKA soil-moisture CSV with robust encoding handling.

    Some RIKA exports are UTF-16LE without a BOM. In that case the file contains
    many null bytes and pandas' default UTF-8 reader fails on the degree symbol
    (°C). This helper detects that format and falls back safely.
    """
    filepath = Path(filepath)
    raw = filepath.read_bytes()

    # UTF-16LE export without BOM: visible as N\x00o\x00;\x00T\x00...
    if raw[:300].count(b"\x00") > 50:
        return pd.read_csv(filepath, sep=";", encoding="utf-16le")

    # Normal text exports. Try common encodings in order.
    last_error = None
    ...
    raise UnicodeDecodeError(
        "unknown", raw, 0, min(len(raw), 1),
        f"Could not decode {filepath}; last error: {last_error}"
    )
...
def load_soil_sensor_csv(
    ...
    df = _read_rika_csv(filepath)
    # Clean possible hidden BOM/whitespace in column names.
    df.columns = [str(c).replace("\ufeff", "").strip() for c in df.columns]

    if "Timestamp" not in df.columns:
        raise ValueError(
            f"No Timestamp column in {filepath}. Columns found: {df.columns.tolist()}"
        )
...
def load_cam_soil_moisture(
    ...
    # The two sensors are not always recorded at exactly the same second.
    # Resample each sensor first so concat does not produce alternating NaN rows.
    shallow_1min = shallow.resample("1min").mean().interpolate(
        "time", limit=5, limit_direction="both"
    )
    deep_1min = deep.resample("1min").mean().interpolate(
        "time", limit=5, limit_direction="both"
    )

    df = pd.concat([
        shallow_1min.rename(columns={
            "moisture_pct": "theta_shallow_pct",
            "theta": "theta_shallow",
            "soil_temp_C": "soil_temp_shallow_C",
        }),
        deep_1min.rename(columns={
    ...
    print(f"  Valid shallow : {int(df['theta_shallow'].notna().sum())} rows")
    print(f"  Valid deep    : {int(df['theta_deep'].notna().sum())} rows")
    return df
...
def load_licor_file(filepath: Union[str, Path]) -> pd.DataFrame:
    ...
    if filepath.suffix.lower() in {".xlsx", ".xlsm", ".xls"}:
        ...
        if not candidates:
            for sep in ["\t", ",", ";"]:
                try:
                    raw = pd.read_csv(filepath, header=None, sep=sep, engine="python", on_bad_lines="skip")
                    table = _scan_table_for_header(raw, required_hint="gsw")
                    if table is not None:
                        candidates.append(table)
                        break
...
def build_cam_rs_profile_from_licor(
    ...
    """
    Build CAM phase r_s profile from one or more LI-COR files.

    Classification uses filename timestamp:
        night:           hour >= 18.5 or hour < 6.0
        midday:          8.0 <= hour < 15.5
        late_afternoon:  15.5 <= hour < 18.5

    If a phase has no usable LI-COR data, fallback value is used.
    """
    phase_values: Dict[str, List[float]] = {"night": [], "midday": [], "late": []}
    summaries = []

    for fp in licor_files:
        fp = Path(fp)
        if not fp.exists():
            print(f"WARNING: LI-COR file not found, skipped: {fp}")
            continue
        try:
            ...
            gsw_valid = gsw[(gsw > 0.001) & (gsw < 5.0)]
            if len(gsw_valid) == 0:
                summaries.append(f"{fp.name}: no usable gsw")
                continue
            r_s = gsw_to_r_s_s_m(gsw_valid.to_numpy())
            r_s_med = float(np.nanmedian(r_s))

            start = df["session_start"].dropna()
            if len(start) > 0:
                ts = pd.Timestamp(start.iloc[0])
                hour = ts.hour + ts.minute / 60.0
            else:
                # Unknown time: do not force it into a phase.
                summaries.append(f"{fp.name}: usable gsw but unknown phase")
                continue
            ...
            if hour >= 18.5 or hour < 6.0:
                ...
                # sunrise transition; treat closer to night but record clearly.
                phase = "night"

            phase_values[phase].append(r_s_med)
            summaries.append(
                f"{fp.name}: phase={phase}, n={len(gsw_valid)}, "
                f"gsw_mean={gsw_valid.mean():.3f}, r_s_median={r_s_med:.1f} s/m"
            )
        except Exception as exc:
            summaries.append(f"{fp.name}: skipped ({exc})")

    r_night = float(np.median(phase_values["night"])) if phase_values["night"] else fallback.r_s_night_s_m
    r_mid = float(np.median(phase_values["midday"])) if phase_values["midday"] else fallback.r_s_midday_s_m
    r_late = float(np.median(phase_values["late"])) if phase_values["late"] else fallback.r_s_late_afternoon_s_m

    # Midday should normally be more resistant than late afternoon/night. If no midday
    # file exists and late file exists, keep fallback midday unless it is lower than late.
    r_mid = max(r_mid, r_late * 1.4, r_night * 2.0)

    return CAMRsProfile(
        r_s_night_s_m=r_night,
        r_s_midday_s_m=r_mid,
        r_s_late_afternoon_s_m=r_late,
        source_summary="\n".join(summaries) if summaries else fallback.source_summary,
    )
...
def cam_phase_r_s(timestamp: pd.Timestamp, profile: CAMRsProfile) -> float:
    """
    CAM phase r_s profile.

    Phase schedule used as first defensible approximation:
        18:30–05:30  Phase I   night open
        05:30–08:00  transition to closure
        08:00–15:30  Phase III midday closure
        15:30–18:30  Phase IV  late-afternoon partial reopening
    """
    h = timestamp.hour + timestamp.minute / 60.0 + timestamp.second / 3600.0
    r_n = profile.r_s_night_s_m
    r_m = profile.r_s_midday_s_m
    r_l = profile.r_s_late_afternoon_s_m

    if h >= 18.5 or h < 5.5:
        return float(r_n)
    if 5.5 <= h < 8.0:
        w = (h - 5.5) / (8.0 - 5.5)
        return float((1 - w) * r_n + w * r_m)
    if 8.0 <= h < 15.5:
        return float(r_m)
    if 15.5 <= h < 18.5:
        # Move from midday high resistance toward late-afternoon observed value.
        w = (h - 15.5) / (18.5 - 15.5)
        return float((1 - w) * r_m + w * r_l)
    return float(r_n)
...
def compute_stomatal_resistance_cam(
    ...
    LI-COR-derived CAM phase r_s is used as the physiological base, then adjusted
    with a bounded soil moisture stress. The returned gsw is the equivalent model
    conductance.
    """
    base_rs = cam_phase_r_s(timestamp, rs_profile)
    f_theta = soil_moisture_stress(theta_avg, substrate)
    r_s = max(base_rs * f_theta, 5.0)

    h = timestamp.hour + timestamp.minute / 60.0
    if h >= 18.5 or h < 5.5:
        phase = "I_night_open"
    elif 5.5 <= h < 8.0:
        phase = "II_morning_transition"
    elif 8.0 <= h < 15.5:
        phase = "III_midday_closure"
    else:
        phase = "IV_late_afternoon_reopening"

    gsw_equiv = r_s_to_gsw_mol_m2_s(r_s)
    return float(r_s), float(gsw_equiv), phase
...
def run_cam_simulation(
    ...
    if verbose:
        print("\n" + "=" * 72)
        print("CAM PHYSICAL SIMULATION — LI-COR r_s VERSION")
        print("=" * 72)
        print(f"r_s night          : {rs_profile.r_s_night_s_m:.1f} s/m")
        print(f"r_s midday         : {rs_profile.r_s_midday_s_m:.1f} s/m")
        print(f"r_s late afternoon : {rs_profile.r_s_late_afternoon_s_m:.1f} s/m")
    ...
    results = {
        ...
        "T_a": [], "RH": [], "G_sol": [], "u": [],
        "T_in_used": [],
        "T_f": [], "T_g_top": [], "T_g_mid": [], "T_g_bot": [], "T_s_in": [],
