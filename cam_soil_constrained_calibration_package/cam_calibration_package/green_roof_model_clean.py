...
This copy is packaged with the CAM soil-moisture-constrained calibration runner.
...
def compute_plant_cooling_flux(
    ...
    if "soil_moisture" in df.columns:
        ...
        # Avoid RuntimeWarning: All-NaN slice encountered. This can happen when
        # the soil files do not overlap the selected CAM window, or when the
        # merged input has a soil_moisture column but every value is NaN. In that
        # case, the thermal model should still run and simply ignore moisture
        # limitation by using a neutral factor of 1.0.
        finite_sm = sm[np.isfinite(sm)]
        if finite_sm.size == 0:
            moisture_factor = np.ones(n, dtype=float)
        else:
            if np.max(finite_sm) > 1.5:
                sm = sm / 100.0
