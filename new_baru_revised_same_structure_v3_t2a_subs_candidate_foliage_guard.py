...
─────────────────────────────────────────────────────────────────────────────
[MAP] T2A tidak dipakai sebagai roof-out CAM. Berdasarkan diagnosis Jan dan
      Mar-Apr, T2A diperlakukan sebagai candidate lower/wet substrate CAM
      dan hanya dipakai sebagai supporting diagnostic karena sinyalnya sangat
      teredam serta punya riwayat anomali.
...
[C6] T2A (candidate lower/wet CAM substrate) anomali difilter
     → 66 data dengan nilai tidak fisik pada channel outdoor CAM
     → Penyebab kemungkinan: sensor disconnect sementara
...
# Jadwal penyiraman aktual eksperimen.
# Catatan: ini hanya membuat flag dan/atau water input jika irrigation_mm_per_min > 0.
# Kalau debit belum diukur, biarkan irrigation_mm_per_min=0 agar model fisik tidak
# mengarang jumlah air, tetapi flag tetap bisa diekspor untuk ANN/residual model.
DEFAULT_IRRIGATION_WINDOWS = (("07:00", "07:04"), ("16:30", "16:34"))

def _parse_hhmm_to_minutes(value: str) -> int:
    hh, mm = str(value).split(":")[:2]
    return int(hh) * 60 + int(mm)

def irrigation_flux_at_timestamp(ts,
                                 windows=None,
                                 irrigation_mm_per_min: float = 0.0) -> Tuple[float, int]:
    """Return scheduled irrigation flux [kg/m²s] and flag at a timestamp.

    1 mm water over 1 m² = 1 kg/m². Therefore mm/min / 60 = kg/m²s.
    The end time is exclusive: 07:00–07:04 covers 07:00, 07:01, 07:02, 07:03.
    """
    if windows is None:
        return 0.0, 0
    ts = pd.Timestamp(ts)
    ...
    for start, end in windows:
        a = _parse_hhmm_to_minutes(start)
        b = _parse_hhmm_to_minutes(end)
        if a <= b:
            inside = (minute >= a) and (minute < b)
        else:  # window crosses midnight
            inside = (minute >= a) or (minute < b)
        if inside:
            flux = max(float(irrigation_mm_per_min), 0.0) / 60.0
            return flux, 1
    return 0.0, 0
...
def _trapz_compat(y, dx=1.0):
    """Compatibility wrapper for NumPy versions with/without np.trapezoid."""
...
def load_NI_sensor_data(filepath: str) -> pd.DataFrame:
    ...
    [C6] T2A (candidate lower/wet CAM substrate) difilter dari 66 anomali ekstrem (< -10°C atau > 80°C).
    ...
    Col 14 → 2Ke   : RR Lantai
    Col 15 → T1A   : Atap Outdoor C3 / di bawah substrat C3
    Col 16 → T2A   : Candidate lower/wet substrate CAM → [C6: ada 66 anomali!]
    Col 17 → T2A2  : Atap Indoor CAM      → T_s_in (kalibrasi bromelia)
    Col 18 → T1Tb  : Ruangan RR
    Col 19 → T1Ta  : Atap Outdoor RR
    Col 20 → T1Ta2 : Lower-area substrate CAM (not exact interface) → T_g_lower_CAM
    """
    ...
    # [C6] Filter anomali T2A — candidate lower/wet CAM substrate, 66 data dengan nilai ekstrem
    ...
    df['T_g_top_CAM'] = df['T1Ke']    # Tanah atas CAM → kalibrasi/diagnostik bromelia

    # IMPORTANT mapping decision after Jan + Mar-Apr diagnostics:
    # T2A is NOT treated as roof-out. It is a candidate lower/wet substrate
    # sensor because it is very damped and has weak similarity to roof-side signals.
    # It also has anomaly history, so use it as supporting diagnostic only.
    df['T_g_lower_wet_CAM_candidate'] = df['T2A']

    # T1Ta2 is a lower-area substrate sensor, but not necessarily the exact
    # substrate-slab interface. Keep a backward-compatible alias, but do not
    # over-interpret it as the exact bottom boundary.
    df['T_g_lower_CAM'] = df['T1Ta2']
    df['T_g_bot_CAM'] = df['T_g_lower_CAM']

    # Main validation target for CAM inner roof.
    df['T_s_in_CAM']  = df['T2A2']

    df['T_s_ext_C3']  = df['T1A']     # Atap outdoor/roof-side C3 / di bawah substrat C3
    ...
    # Referensi RR / conventional roof
    df['T_r_in_RR']    = df['T2Ka']   # Atap indoor RR
    df['T_r_floor_RR'] = df['2Ke']    # Lantai RR
    df['T_room_RR']    = df['T1Tb']   # Ruangan RR
    df['T_r_ext_RR']   = df['T1Ta']   # Atap outdoor RR
    ...
    for col, label in [
        ...
        ('T_g_lower_CAM', 'T_g_lower CAM (T1Ta2, not exact interface)'),
        ('T_g_lower_wet_CAM_candidate', 'T_g_lower wet CAM candidate (T2A)'),
        ('T_s_in_CAM',  'T_s_in CAM (T2A2)'),
        ('T_s_ext_C3',  'T_s_ext C3 (T1A)'),
...
def solve_foliage_temperature(T_f_prev: float,
                              ...
                              dt: float) -> float:
    """
    Robust foliage temperature solver.

    Why this patch exists:
    The previous linearized implicit update could generate impossible CAM foliage
    temperatures, e.g. 8°C at night or >60°C during daytime while ambient was
    ~24–34°C. That is not acceptable for this tropical test-cell experiment.

    This version solves the foliage energy-balance residual inside a physically
    plausible bracket, then relaxes toward that bounded equilibrium using an
    approximate leaf heat capacity. The bracket is not meant to force fitting;
    it prevents the plant submodel from feeding unphysical temperatures into
    the substrate/ET calculation.
    """
    sigma   = 5.67e-8
    rho_air = 1.2
    cp_air  = 1005.0
    ...
    # Leaf temperature physical guard for tropical outdoor/test-cell data.
    # Leaf temperature may be a few °C below air by radiative cooling, but it
    # should not fall far below dew point when condensation is not modelled.
    T_dew_C = dew_point_C(T_a_C, RH)
    lower_C = max(T_a_C - 8.0, T_dew_C - 2.0)
    upper_C = T_a_C + (18.0 if G_sol > 50 else 6.0)
    T_low  = lower_C + 273.15
    T_high = upper_C + 273.15

    # Use clipped previous temperature only for property evaluation/relaxation.
    T_prev = float(np.clip(T_f_prev, T_low, T_high))

    epsilon_fg = 1.0 / (1.0/plant.epsilon_f + 1.0/substrate.epsilon_g - 1.0)

    def residual(T_K: float) -> float:
        """Positive residual means foliage still has net heat gain [W/m²]."""
        P_f_sat = saturation_pressure(T_K)
        r_stoma = compute_stomatal_resistance(
            G_sol=G_sol,
            T_f_K=T_K,
            P_f_sat=P_f_sat,
            P_a=P_a,
        ...
        R_f_net = (
            plant.alpha_f * G_sol
            - plant.epsilon_f * sigma * (T_K**4 - T_sky**4)
            - epsilon_fg * sigma * (T_K**4 - T_g_surface**4)
        )
        H  = h_conv_f * (T_K - T_a_K)
        LE = h_eva_f * max(P_f_sat - P_a, 0.0)
        return R_f_net - H - LE

    # Bisection inside bracket. If the residual does not cross zero, select the
    # bounded temperature with the smallest absolute residual.
    ...
    if np.isfinite(f_low) and np.isfinite(f_high) and f_low * f_high <= 0:
        ...
        grid = np.linspace(T_low, T_high, 31)
        vals = np.array([abs(residual(T)) if np.isfinite(residual(T)) else np.inf for T in grid])
        T_eq = float(grid[int(np.argmin(vals))])

    # Approximate foliage heat capacity per ground area [J/m²K]. CAM leaves are
    # thicker/succulent, so use larger heat capacity than C3.
    C_leaf_single = 1800.0 if plant.plant_type == "CAM" else 900.0
    C_leaf_area = C_leaf_single * max(float(plant.LAI), 0.2)

    # Estimate conductance for relaxation timescale.
    P_prev = saturation_pressure(T_prev)
    r_stoma_prev = compute_stomatal_resistance(
        G_sol, T_prev, P_prev, P_a, theta_avg, plant, substrate
    )
    h_conv_prev = plant.LAI * (rho_air * cp_air) / r_a
    h_eva_prev  = plant.LAI / gamma * (rho_air * cp_air) / (r_a + r_stoma_prev)
    dPdT = saturation_pressure(T_prev + 0.5) - saturation_pressure(T_prev - 0.5)
    h_rad = 4.0 * plant.epsilon_f * sigma * max(T_prev, 250.0)**3
    G_total = max(h_conv_prev + h_eva_prev * dPdT + h_rad, 1.0)
    tau = C_leaf_area / G_total
    relax = float(np.clip(dt / (tau + dt), 0.0, 1.0))

    T_new = T_prev + relax * (T_eq - T_prev)
    return float(np.clip(T_new, T_low, T_high))
...
def run_simulation(weather_df: pd.DataFrame,
                   ...
                   irrigation_windows: Optional[Sequence[Tuple[str, str]]] = None,
                   irrigation_mm_per_min: float = 0.0,
    ...
    if irrigation_windows is not None:
        print(f"Irrigation schedule: {irrigation_windows} | flux={irrigation_mm_per_min:.3f} mm/min")
    print(f"{'='*60}")
    ...
    for step, (ts, row) in enumerate(weather_1s.iterrows()):
        ...
        j_rain = max(float(row.get('rain_flux', 0.0)), 0.0)
        j_irr_sched, irrigation_flag = irrigation_flux_at_timestamp(
            ts, windows=irrigation_windows, irrigation_mm_per_min=irrigation_mm_per_min
        )
        j_irr_const = max(float(j_irrigation), 0.0)
        if j_irr_const > 0:
            irrigation_flag = 1
        j_pr  = j_rain + j_irr_const + j_irr_sched
        ...
        if step % int(save_every_s/dt) == 0:
            ...
            results['j_rain'].append(float(j_rain))
            results['j_irrigation'].append(float(j_irr_const + j_irr_sched))
            results['irrigation_flag'].append(int(irrigation_flag))
    ...
    # Q_gain — Persamaan (18)
    results['Q_gain'] = float(_trapz_compat(results['q_s_in'], dx=save_every_s))  # J/m²
...
def validation_metrics(sim: pd.Series, obs: pd.Series) -> Dict[str, float]:
    ...
    if len(common) == 0:
        raise ValueError("Tidak ada timestamp yang overlap antara simulasi dan observasi.")
    diff = (sim.loc[common] - obs.loc[common]).dropna()
    if diff.empty:
        raise ValueError("Overlap timestamp ada, tetapi residual kosong setelah drop NaN.")
    return {
        'n': int(diff.count()),
        'bias_C': float(diff.mean()),
        'mae_C': float(diff.abs().mean()),
        'rmse_C': float(np.sqrt((diff**2).mean())),
        'max_error_C': float(diff.max()),
        'min_error_C': float(diff.min()),
    }
...
def calibrate_LAI(weather_df: pd.DataFrame,
                  ...
                  T_s_in_initial_C: Optional[float] = None,
                  irrigation_windows: Optional[Sequence[Tuple[str, str]]] = None,
                  irrigation_mm_per_min: float = 0.0) -> Tuple[float, dict]:
    """
    Kalibrasi LAI berbasis timestamp.

    Target default:
    - CAM: T_s_in_CAM (T2A2) sebagai target utama; T_g_top_CAM bisa dipakai nanti
    - C3 : T_s_in_C3  (T3Ka)
...
def run_validation_case(plant_type: str,
                        ...
                        irrigation_windows: Optional[Sequence[Tuple[str, str]]] = DEFAULT_IRRIGATION_WINDOWS,
                        irrigation_mm_per_min: float = 0.0) -> Tuple[dict, Dict[str, float]]:
...
# hahahahah ganti benrar
    ...
    if plant_type == 'CAM':
        ...
        # Default: T2A2 sesuai mapping terbaru
        target_col = VALIDATION_TARGETS.get("CAM", "T2A2")
...
def export_ann_residual_dataset(results: dict,
                                ni: pd.DataFrame,
                                target_col: str,
                                out_path: Optional[str] = None,
                                plant_type: Optional[str] = None) -> pd.DataFrame:
    """Build a 1-minute dataset for ANN residual correction.

    Target residual convention:
        residual_C = measured - physics_model
    so the corrected model is:
        T_hybrid = T_physics + ANN_residual
    """
    sim = pd.Series(results['T_s_in'],
                    index=pd.to_datetime(results['datetime']),
                    name='T_physics_C').sort_index().resample('1min').mean()

    if target_col not in ni.columns:
        raise ValueError(f"target_col '{target_col}' tidak ditemukan di NI data.")
    obs = ni[target_col].sort_index().resample('1min').mean().rename('T_measured_C')

    driver_cols = [
        'T_a', 'G_sol', 'T_in_used', 'theta_top', 'theta_mid', 'theta_bot',
        'theta_mean', 'j_pr', 'j_rain', 'j_irrigation', 'irrigation_flag',
        'j_eva', 'q_s_in', 'T_f', 'T_g_top', 'T_g_mid', 'T_g_bot'
    ]
    data = {'T_physics_C': sim, 'T_measured_C': obs}
    for col in driver_cols:
        if col in results:
            data[col] = pd.Series(results[col],
                                  index=pd.to_datetime(results['datetime']),
                                  name=col).sort_index().resample('1min').mean()

    df = pd.concat(data.values(), axis=1).dropna(subset=['T_physics_C', 'T_measured_C'])
    df['residual_C'] = df['T_measured_C'] - df['T_physics_C']
    df['hour'] = df.index.hour + df.index.minute / 60.0
    df['sin_hour'] = np.sin(2*np.pi*df['hour']/24.0)
    df['cos_hour'] = np.cos(2*np.pi*df['hour']/24.0)
    if plant_type is not None:
        df['plant_type'] = str(plant_type).upper()

    df_out = df.reset_index().rename(columns={'index': 'datetime'})
    if out_path is not None:
        df_out.to_csv(out_path, index=False)
        print(f"ANN residual dataset saved: {out_path} | rows={len(df_out)}")
    return df_out
