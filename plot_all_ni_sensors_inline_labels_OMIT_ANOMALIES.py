...
This version omits anomalies before plotting. It avoids openpyxl entirely and
parses the XLSX XML directly, so it also avoids the openpyxl Fill error.

Use:
    python plot_all_ni_sensors_inline_labels_OMIT_ANOMALIES.py
...
# Optional: if some LabVIEW channels are in Fahrenheit, set True.
# Keep False unless you are sure, because exposed roof surfaces can also be high in °C.
AUTO_CONVERT_F_TO_C = False

# Omit anomalies from plot/statistics by replacing them with NaN.
# No interpolation is applied; anomalous points are simply not drawn.
OMIT_ANOMALIES = True

# Hard physical limits for expected °C NI temperature channels.
# For outdoor roof surfaces, 60–65 °C can still be possible; >65 °C is flagged.
PHYSICAL_MIN_C = -10.0
PHYSICAL_MAX_C = 65.0

# Spike detector: a single point is flagged when both adjacent jumps are too large.
# Increase this if your sampling interval is coarse or exposed roof sensors change very fast.
MAX_ISOLATED_JUMP_C = 6.0

# Hampel/rolling-median detector for local outliers.
# Window is in samples after loading raw data. With 1-min NI data, 15 ≈ 15 minutes.
HAMPEL_WINDOW = 15
HAMPEL_N_SIGMA = 5.0
...
def load_multiple_ni(files) -> pd.DataFrame:
    ...
    if OMIT_ANOMALIES:
        df, report = omit_sensor_anomalies(df)
        report_path = OUTPUT_DIR / "sensor_anomaly_report.csv"
...
# =============================================================================
# ANOMALY CLEANING — OMIT, DO NOT INTERPOLATE
# =============================================================================

def _hampel_bad_points(s: pd.Series, window: int = HAMPEL_WINDOW, n_sigma: float = HAMPEL_N_SIGMA) -> pd.Series:
    """Return a boolean mask for local outliers using rolling median + MAD.

    The method is robust for isolated sensor spikes. It is intentionally not used
    to remove long physical trends, because the median follows slow changes.
    """
    s = pd.to_numeric(s, errors="coerce")
    med = s.rolling(window=window, center=True, min_periods=max(5, window // 3)).median()
    abs_dev = (s - med).abs()
    mad = abs_dev.rolling(window=window, center=True, min_periods=max(5, window // 3)).median()
    robust_sigma = 1.4826 * mad.replace(0, np.nan)
    bad = abs_dev > (n_sigma * robust_sigma)
    return bad.fillna(False)


def _isolated_jump_bad_points(s: pd.Series, max_jump: float = MAX_ISOLATED_JUMP_C) -> pd.Series:
    """Flag isolated one-sample spikes based on both previous and next jumps.

    A point is considered suspicious only when it jumps away from both neighbors.
    This prevents normal daytime heating/cooling ramps from being removed.
    """
    s = pd.to_numeric(s, errors="coerce")
    prev_jump = (s - s.shift(1)).abs()
    next_jump = (s - s.shift(-1)).abs()
    neighbor_span = (s.shift(1) - s.shift(-1)).abs()

    bad = (prev_jump > max_jump) & (next_jump > max_jump) & (neighbor_span < max_jump)
    return bad.fillna(False)


def omit_sensor_anomalies(df: pd.DataFrame) -> tuple[pd.DataFrame, pd.DataFrame]:
    """Replace anomalous sensor values with NaN and return a report.

    Removed points are omitted from the graph and from summary statistics.
    Nothing is interpolated, so the plot keeps gaps where data was bad.
    """
    clean = df.copy()
    report_rows = []
    ...
    for col in SENSOR_COLS:
        ...
        if n_total == 0:
            report_rows.append({
                ...
                "n_total": 0,
                "n_physical_bad": 0,
                "n_isolated_jump_bad": 0,
                "n_hampel_bad": 0,
                "n_total_omitted": 0,
                "percent_omitted": np.nan,
                "min_after": np.nan,
                "max_after": np.nan,
            })
            continue

        physical_bad = (s < PHYSICAL_MIN_C) | (s > PHYSICAL_MAX_C)
        jump_bad = _isolated_jump_bad_points(s)
        hampel_bad = _hampel_bad_points(s)

        bad = (physical_bad | jump_bad | hampel_bad).fillna(False)
        clean.loc[bad, col] = np.nan

        s_after = clean[col].dropna()
        n_bad = int(bad.sum())
        pct_bad = 100.0 * n_bad / n_total if n_total else np.nan

        print(
            f"  {col:5s}: omitted={n_bad:5d}/{n_total:<5d} ({pct_bad:5.1f}%) | "
            f"physical={int(physical_bad.sum()):4d}, "
            f"jump={int(jump_bad.sum()):4d}, "
            f"hampel={int(hampel_bad.sum()):4d}"
        )

        report_rows.append({
            "sensor": col,
            "n_total": n_total,
            "n_physical_bad": int(physical_bad.sum()),
            "n_isolated_jump_bad": int(jump_bad.sum()),
            "n_hampel_bad": int(hampel_bad.sum()),
            "n_total_omitted": n_bad,
            "percent_omitted": pct_bad,
            "min_after": float(s_after.min()) if not s_after.empty else np.nan,
            "max_after": float(s_after.max()) if not s_after.empty else np.nan,
        })
