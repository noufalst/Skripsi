"""
CAM hypothesis simulation sweep with weather overlay:
...
Expected files in:
    outputs_cam_t1ka_tin_proxy_sweep_weather/
...
BASE_DIR = SCRIPT_DIR
OUTPUT_DIR = SCRIPT_DIR / "outputs_cam_t1ka_tin_proxy_sweep_weather"
...
def plot_overlay(all_results, ni, target_col, weather=None):
    ...
    ax = axes[0]
    ax.plot(obs.index, obs.values, label=f"Measured target {target_col}", linewidth=2.6)
    for tag, results in all_results.items():
        sim = make_series(results, "T_s_in")
        ax.plot(sim.index, sim.values, linestyle="--", linewidth=1.5, label=tag)
    ax.set_title("CAM hypothesis overlay: T1Ka as T_s_in target, different T_in proxies + weather")
