...
If your CAM wiring was reversed, run:
    python check_cam_soil_moisture_daytime.py --swap-depths
...
If sensor 1 is actually the deep CAM sensor and sensor 2 is shallow:
    python check_cam_soil_moisture_daytime.py --swap-depths

If the weather Excel is missing, the script still runs using local daytime hours
06:00-18:00 instead of solar radiation.

This version also auto-detects whether RIKA timestamps should be treated as
UTC/GMT converted to WIB or already-local timestamps, then prints available
date ranges before filtering. If the requested window has no overlap, it can
auto-use the available overlap instead of crashing.
"""
...
CAM_MEMBER_SENSOR1 = "sensor 1 COM5_CAM.csv"
CAM_MEMBER_SENSOR2 = "sensor 2 COM6_CAM.csv"


def cam_members_by_depth(swap_depths: bool = False) -> Dict[str, str]:
    """Return CAM ZIP member mapping by depth label.

    Default assumption:
        sensor 1 -> shallow, sensor 2 -> deep

    If swap_depths=True:
        sensor 1 -> deep, sensor 2 -> shallow
    """
    if swap_depths:
        return {
            "deep": CAM_MEMBER_SENSOR1,
            "shallow": CAM_MEMBER_SENSOR2,
        }
    return {
        "shallow": CAM_MEMBER_SENSOR1,
        "deep": CAM_MEMBER_SENSOR2,
    }
...
class CheckConfig:
    ...
    swap_depths: bool = False             # True if CAM sensor 1 is deep and sensor 2 is shallow
    timestamp_mode: str = "auto"          # auto, utc_to_local, or local
    auto_window: bool = True              # if selected window is empty, use available overlap
...
def _parse_rika_timestamp(timestamp_col: pd.Series, timezone: str, mode: str) -> pd.Series:
    """Parse RIKA timestamps.

    mode='utc_to_local' treats logger timestamps as UTC/GMT and converts to WIB.
    mode='local' treats logger timestamps as already local clock time.
    """
    if mode == "utc_to_local":
        ts_utc = pd.to_datetime(timestamp_col, utc=True, errors="coerce")
        return ts_utc.dt.tz_convert(timezone).dt.tz_localize(None)
    if mode == "local":
        ts = pd.to_datetime(timestamp_col, errors="coerce")
        # If pandas produced a timezone-aware datetime64 series, normalize to local-naive.
        try:
            ...
            pass
        return ts
    raise ValueError("timestamp mode must be 'utc_to_local' or 'local'")


def _choose_timestamp_mode(raw_tables: Dict[str, pd.DataFrame], timezone: str, start: str, end: str) -> str:
    """Choose timestamp interpretation with the largest overlap in the requested window."""
    start_ts = pd.Timestamp(start)
    end_ts = pd.Timestamp(end)
    scores = {}
    ranges = {}
    for mode in ["utc_to_local", "local"]:
        total = 0
        min_ts = []
        max_ts = []
        for raw in raw_tables.values():
            ts = _parse_rika_timestamp(raw["Timestamp"], timezone, mode)
            valid = ts.dropna()
            if len(valid):
                min_ts.append(valid.min())
                max_ts.append(valid.max())
            total += int(((ts >= start_ts) & (ts <= end_ts)).sum())
        scores[mode] = total
        ranges[mode] = (min(min_ts) if min_ts else None, max(max_ts) if max_ts else None)

    print("Timestamp mode overlap check:")
    for mode in ["utc_to_local", "local"]:
        a, b = ranges[mode]
        print(f"  {mode:12s}: {scores[mode]} points in selected window | range {a} -> {b}")

    # Prefer the mode with more selected-window points. If tied, keep utc_to_local
    # because that matches the main model loader assumption.
    return "local" if scores["local"] > scores["utc_to_local"] else "utc_to_local"
...
def load_cam_soil_moisture(
    ...
    swap_depths=False: sensor 1 -> shallow, sensor 2 -> deep.
    swap_depths=True : sensor 1 -> deep, sensor 2 -> shallow.
    """
    if not zip_path.exists():
        raise FileNotFoundError(f"Soil moisture ZIP not found: {zip_path}")

    members = cam_members_by_depth(swap_depths=swap_depths)

    frames = []
    with zipfile.ZipFile(zip_path, "r") as z:
        available = set(z.namelist())
        missing = [m for m in members.values() if m not in available]
        if missing:
        ...
        print("CAM depth mapping:")
        for label, member in members.items():
            print(f"  {label:7s} <- {member}")
        ...
        for label, member in members.items():
            ...
            raw_tables[label] = raw

        mode = timestamp_mode
        if mode == "auto":
            mode = _choose_timestamp_mode(raw_tables, timezone, start, end)
        if mode not in {"utc_to_local", "local"}:
            raise ValueError("--timestamp-mode must be auto, utc_to_local, or local")
        print(f"Using RIKA timestamp mode: {mode}")

        for label, raw in raw_tables.items():
            ts_local = _parse_rika_timestamp(raw["Timestamp"], timezone, mode)

            moisture_col = "Moisture (%)"
            temp_col = "Temperature"
            if moisture_col not in raw.columns:
                raise ValueError(f"{members[label]} has no '{moisture_col}' column")
    ...
    if not out.empty:
        print(f"Loaded CAM soil range: {out.index.min()} -> {out.index.max()} | {len(out)} rows")
...
def load_weather_excel(weather_path: Path, sheet_name: str = DEFAULT_WEATHER_SHEET) -> Optional[pd.DataFrame]:
    ...
    if not df.empty:
        print(f"Loaded weather range : {df.index.min()} -> {df.index.max()} | {len(df)} rows")
...
def plot_diagnostic(df: pd.DataFrame, summary: pd.DataFrame, cfg: CheckConfig, save_path: Path) -> None:
    ...
    if "theta_shallow" in df_plot:
        ax.plot(df_plot.index, df_plot["theta_shallow"], label="theta shallow assumed ~2 cm", linewidth=1.8)
    if "theta_deep" in df_plot:
        ax.plot(df_plot.index, df_plot["theta_deep"], label="theta deep assumed ~7 cm", linewidth=1.8)
    ax.set_ylabel("VWC θ (-)")
    ...
    shade_daytime(ax, df_plot)
    for col, label in [
        ("dtheta_shallow_per_h_30min", "dθ shallow-assumed/dt, 30-min"),
        ("dtheta_deep_per_h_30min", "dθ deep-assumed/dt, 30-min"),
    ]:
        if col in df_plot:
...
def plot_solar_vs_drying(df: pd.DataFrame, cfg: CheckConfig, save_path: Path) -> None:
    ...
    plotted = False
    for col, label in [
        ("dtheta_shallow_per_h_30min", "shallow assumed ~2 cm"),
        ("dtheta_deep_per_h_30min", "deep assumed ~7 cm"),
    ]:
        if col in d:
...
def parse_args() -> CheckConfig:
    ...
    parser.add_argument(
        "--swap-depths",
        action="store_true",
        help="Use this if CAM sensor 1 is deep and CAM sensor 2 is shallow.",
    )
    parser.add_argument(
        "--timestamp-mode",
        choices=["auto", "utc_to_local", "local"],
        default="auto",
        help="How to parse RIKA Timestamp: auto, utc_to_local, or local.",
    )
    parser.add_argument(
        "--no-auto-window",
        dest="auto_window",
        action="store_false",
        help="Disable automatic fallback to available data range when selected window is empty.",
    )
...
def describe_range(name: str, df: Optional[pd.DataFrame]) -> str:
    if df is None:
        return f"{name}: not loaded"
    if df.empty:
        return f"{name}: empty"
    return f"{name}: {df.index.min()} -> {df.index.max()} ({len(df)} rows)"


def choose_available_window(soil: pd.DataFrame, weather: Optional[pd.DataFrame]) -> Optional[Tuple[pd.Timestamp, pd.Timestamp]]:
    """Choose overlap between soil and weather; if weather missing, use full soil range."""
    if soil.empty:
        return None
    start = soil.index.min()
    end = soil.index.max()
    if weather is not None and not weather.empty:
        start = max(start, weather.index.min())
        end = min(end, weather.index.max())
...
def main() -> None:
    ...
    print(f"Swap depths    : {cfg.swap_depths}")
    print(f"Timestamp mode : {cfg.timestamp_mode}")
    print(f"Auto window    : {cfg.auto_window}")

    soil = load_cam_soil_moisture(
        soil_zip_path,
        timezone=cfg.timezone,
        swap_depths=cfg.swap_depths,
    ...
    print("\nAvailable ranges before window filter:")
    print("  " + describe_range("soil", soil))
    print("  " + describe_range("weather", weather))
    print(f"  requested window: {cfg.start} -> {cfg.end}")

    df = restrict_window(df, cfg.start, cfg.end)
    if df.empty:
        if cfg.auto_window:
            chosen = choose_available_window(soil, weather)
            if chosen is not None:
                auto_start, auto_end = chosen
                print("\nWARNING: No data inside selected CAM window.")
                print(f"Auto-window enabled, using available overlap: {auto_start} -> {auto_end}")
                cfg.start = str(auto_start)
                cfg.end = str(auto_end)
                if weather is not None:
                    df = soil.join(weather, how="left")
                else:
                    df = soil.copy()
        ...
        if df.empty:
            raise ValueError(
                "No data inside selected CAM window.\n"
                + describe_range("soil", soil) + "\n"
                + describe_range("weather", weather) + "\n"
                + f"requested: {cfg.start} -> {cfg.end}\n"
                + "Try one of these:\n"
                + "  python check_cam_soil_moisture_daytime.py --timestamp-mode local\n"
                + "  python check_cam_soil_moisture_daytime.py --timestamp-mode utc_to_local\n"
                + "  python check_cam_soil_moisture_daytime.py --start YYYY-MM-DD HH:MM:SS --end YYYY-MM-DD HH:MM:SS\n"
                + "  python check_cam_soil_moisture_daytime.py --base-dir path/to/data_folder"
            )
    ...
    interpretation["cam_depth_mapping"] = cam_members_by_depth(cfg.swap_depths)
    interpretation["timestamp_mode_requested"] = cfg.timestamp_mode
    ...
    print("\nNote:")
    print("  A flat assumed-2 cm / assumed-7 cm sensor does not prove zero surface evaporation.")
