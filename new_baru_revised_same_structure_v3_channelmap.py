...
[C6] T2A (Atap outdoor CAM) anomali difilter
...
VALIDATION_WINDOWS: Dict[str, Tuple[pd.Timestamp, pd.Timestamp]] = {
    ...
    "CAM": (pd.Timestamp("2026-03-31 17:58:00"),
...
def load_NI_sensor_data(filepath: str) -> pd.DataFrame:
    ...
    [C6] T2A (Atap outdoor CAM) difilter dari 66 anomali ekstrem (< -10°C atau > 80°C).
    ...
    # [C6] Filter anomali T2A — Atap Outdoor CAM, 66 data dengan nilai ekstrem
    ...
    df['T_g_bot_CAM'] = df['T1Ta2']   # Tanah bawah CAM → validasi profil
    df['T_s_in_CAM']  = df['T2A2']    # Atap indoor CAM → kalibrasi bromelia (dual)
    df['T_s_ext_CAM'] = df['T2A']     # Atap outdoor CAM → difilter [C6]
    df['T_s_ext_C3']  = df['T1A']     # Atap outdoor C3 / di bawah substrat C3
    ...
    for col, label in [
        ...
        ('T_g_bot_CAM', 'T_g_bot CAM (T1Ta2)'),
        ('T_s_in_CAM',  'T_s_in CAM (T2A2)'),
        ('T_s_ext_CAM', 'T_s_ext CAM (T2A)'),
